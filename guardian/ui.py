...
def create_summary_dashboard(
    ...
    findings_box = _create_metric_box(
        "FINDINGS", str(total_findings), "cyan" if total_findings == 0 else "yellow"
    )
    patches_box = _create_metric_box(
        "PATCHES", str(patches_generated), "green" if patches_generated > 0 else "dim"
    )
    ...
    if critical > 0:
        severity_table.add_row(
            ...
            _severity_bar(critical, total_findings, "red"),
        )
    if high > 0:
        severity_table.add_row(
            "[red]HIGH[/red]", f"[red]{high}[/red]", _severity_bar(high, total_findings, "red")
        )
    if medium > 0:
        severity_table.add_row(
            "[yellow]MEDIUM[/yellow]",
            f"[yellow]{medium}[/yellow]",
            _severity_bar(medium, total_findings, "yellow"),
        )
    if low > 0:
        severity_table.add_row(
            "[blue]LOW[/blue]", f"[blue]{low}[/blue]", _severity_bar(low, total_findings, "blue")
        )
    if info > 0:
        severity_table.add_row(
            "[dim]INFO[/dim]", f"[dim]{info}[/dim]", _severity_bar(info, total_findings, "dim")
        )
...
def print_findings_table(
    findings: list[dict], title: str = "Security Findings", max_items: int = 15
) -> None:
...
def print_patches_summary(findings_with_patches: list, max_items: int = 5) -> None:
    ...
    patches = [
        f
        for f in findings_with_patches
        if f.get("patch_suggestions") or hasattr(f, "patch_suggestions")
    ]
    ...
    for f in patches[:max_items]:
        ...
        if patch:
            ...
            confidence = (
                patch.confidence if hasattr(patch, "confidence") else patch.get("confidence", 0)
            )
            conf_color = "green" if confidence > 0.7 else "yellow" if confidence > 0.4 else "red"
            table.add_row(location, source, f"[{conf_color}]{confidence:.0%}[/{conf_color}]")
...
def print_analysis_complete(
    ...
    if high_risk > 0:
        console.print(
            f"[bold red]{Icons.FIRE} {high_risk} high-risk findings require immediate attention[/bold red]"
        )
        if patches > 0:
            console.print(
                f"[green]  {Icons.PATCH} {patches} patches available - review suggested fixes[/green]"
            )
    elif patches > 0:
        console.print(
            f"[green]{Icons.CHECK} Analysis complete - {patches} patches ready for review[/green]"
        )
    else:
