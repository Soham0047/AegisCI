...
def _extract_location_key(finding: dict[str, Any], scanner: str) -> tuple[str, int, int]:
    ...
    if scanner == "bandit":
        return (
            ...
            finding.get("line_number", 0) + finding.get("line_range", [0])[-1] - 1
            if finding.get("line_range")
            else finding.get("line_number", 0),
        )
    ...
    for (file_path, _), scanner_findings in location_index.items():
        ...
        for scanner_name, finding in scanner_findings:
            ...
            if scanner_name == "bandit":
                ...
                messages.append(
                    f"CVE: {finding.get('cve_id', 'Unknown')} - {finding.get('description', '')}"
                )
        ...
        final_severity = max(
            ...
            key=lambda s: {"CRITICAL": 4, "HIGH": 3, "MEDIUM": 2, "LOW": 1, "INFO": 0}.get(
                _normalize_severity(s), 2
            ),
        )
        final_confidence = max(
            confidences,
            key=lambda c: {"HIGH": 2, "MEDIUM": 1, "LOW": 0}.get(_normalize_confidence(c), 1),
        )
        final_category = _normalize_category(
            categories[0] if categories else "unknown", scanner_sources[0]
        )
...
def compute_enhanced_features(
    ...
    features = {
        ...
        "has_user_input": any(
            x in code_snippet.lower() for x in ["request", "input", "args", "params", "query"]
        ),
        "has_dangerous_function": any(
            x in code_snippet.lower() for x in ["eval", "exec", "system", "popen", "shell"]
        ),
        "has_sql_keyword": any(
            x in code_snippet.upper() for x in ["SELECT", "INSERT", "UPDATE", "DELETE", "DROP"]
        ),
        "has_file_operation": any(
            x in code_snippet.lower() for x in ["open(", "read(", "write(", "file"]
        ),
    }
