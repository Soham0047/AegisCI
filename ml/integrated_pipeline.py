...
class IntegratedPipeline:
    ...
    def analyze_finding(
        ...
        code_snippet = (
            ...
            finding.get("code") or 
            finding.get("evidence", {}).get("excerpt", "") or
            ""
        )
        file_path = (
            finding.get("file_path") or 
            finding.get("filename") or  # Bandit
            finding.get("location", {}).get("filepath", "") or
            finding.get("path", "") or  # Semgrep
            "unknown"
        )
        line_start = (
            finding.get("line_start") or 
            finding.get("line_number") or  # Bandit
            finding.get("location", {}).get("start_line", 0) or
            finding.get("start", {}).get("line", 0) or  # Semgrep
            0
        )
        line_end = (
            finding.get("line_end") or 
            finding.get("location", {}).get("end_line", line_start) or
            finding.get("end", {}).get("line", line_start) or  # Semgrep
            line_start
        )
        category = (
            finding.get("category") or 
            finding.get("test_id") or  # Bandit
            finding.get("rule", {}).get("category", "") or
            finding.get("check_id", "") or  # Semgrep
            "unknown"
        )
        severity = (
            finding.get("severity") or 
            finding.get("issue_severity", "").lower() or  # Bandit
            "medium"
        )
        finding_id = (
            finding.get("finding_id") or 
            finding.get("sample_id") or 
    ...
    def analyze_batch(
        ...
        return [
            self.analyze_finding(f, generate_patches, top_k_citations)
            for f in findings
        ]
